...
def ensemble(m, params, hess=None, 
    ...
    return ensemble_log_params(m, params, hess, steps, max_run_hours,
                               ...
                               periodic=periodic, recalc_step=recalc_step)
...
def ensemble_log_params(m, params, hess=None, 
    ...
    if periodic:
        curr_F = m.free_energy_periodic(curr_params, temperature)
    else:
    ...
    while steps_attempted < steps+1:
        ...
        try:
            if periodic:
                next_F = m.free_energy_periodic(next_params, temperature)
                print 'next_F = ' + str(next_F)
            else:              
        ...
        if scipy.isnan(next_F): next_F = scipy.inf
        next_hess_set = False
        if (recalc_hess_alg and not scipy.isinf(next_F)
            and steps_attempted % recalc_step == 0):
            try:
        ...
        if accepted:
            ...
            curr_sf = m.internalVars['scaleFactors'].copy()
            curr_F = next_F
            if (recalc_hess_alg and next_hess_set 
                and steps_attempted % recalc_step == 0
                and not scipy.isinf(next_F)):
                hess = next_hess
