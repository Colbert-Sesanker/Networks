...
class Model:
    ...
    def periodic_cost(self, params):
        """
        Return the cost of periodic curve fit
        """
        return self._evaluate(params, periodic_distance = True)   
    ...
    def oscillate_cost(self, params):
        """
        Return the cost of periodic curve fit
        """
        return self._evaluate(params, oscillate_distance = True)
    
    def get_entropy(self, params, T):
        self.ComputeInternalVariables(T)
        entropy = 0
        for expt, sf_ents in self.internalVars['scaleFactor_entropies'].items():
            for group, ent in sf_ents.items():
    ...
    def free_energy_periodic(self, params, T):
        entropy = self.get_entropy(params, T)
        cost    = self.periodic_cost(params)                      
        return cost        
    ...
    def periodic_hessian(self, params, eps=1e-5,
                         relativeScale=True, stepSizeCutoff=1e-6,
                         verbose=False, f0_zero=False):
        """
        Same as hessian except uses periodic cost function
	...
	hess = scipy.zeros((nOv, nOv), scipy.float_)
        ...
        for i in range(nOv):
            for j in range(i, nOv):
                hess[i][j] = self.hessian_elem(self.periodic_cost, f0,
