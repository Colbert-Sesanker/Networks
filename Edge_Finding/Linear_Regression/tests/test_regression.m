...
data = data(:,2:end);             % redefine data without time column 
    % normalize and spline data?
number_nodes = length(data(1,:)); % take length of arbitrary column
nodes = zeros(time_pts, number_nodes); % allocate matrix to hold node time series as columns
