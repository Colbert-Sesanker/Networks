function [dy, flag, new_data] = Repressilator_Positive_AD(t, y, dataStruct)
...
% calls to these hill functions turns out to be costly
...
dy(1) = a4*(A^n4 / (k4^n4 + A^n4)) + a1*(k1^n1 / (k1^n1 + B^n1)) - b1*A + y1;
dy(2) =                              a2*(k2^n2 / (k2^n2 + C^n2)) - b2*B + y2;
dy(3) =                              a3*(k3^n3 / (k3^n3 + A^n3)) - b3*C + y3;
